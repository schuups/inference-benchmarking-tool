"""Builds the §15.1 report notebook programmatically (M9).

`build_notebook()` returns an nbformat NotebookNode whose cells present every
§15.1 panel by calling the tested `analysis`/`plots` modules. The first code cell
...
def build_notebook() -> nbformat.NotebookNode:
    ...
    cells = [
        new_markdown_cell(
            ...
            "Generated from the per-run / centralized results DB (SPECIFICATIONS.md §15.1). "
            "Latency/SLO math is over the measurement phase only (§12.2)."
        ),
        params,
