...
| `model_load_weights_gib` | REAL | GiB read off storage during the weights load (from vLLM's log); with `model_load_weights_s` gives the effective weight-load bandwidth, directly comparable to the §8.1 storage `Parallel read` (§10.2). |
...
| `env` | engine-process environment (not a CLI flag) | Dict of extra env vars rendered into the SLURM EDF `[env]` and the K8s pod env — for vLLM env-only flags with no CLI arg, e.g. `VLLM_ALLOW_LONG_MAX_MODEL_LEN: "1"` to serve a window beyond the model's `config.json` `max_position_embeddings` (used to validate the 256K pipeline on the natively-64K Apertus ahead of Apertus-70B 1.5). Values must be strings. |
