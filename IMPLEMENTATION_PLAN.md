...
✅ done** on `clariden`, and **E3a is ✅ done 2026-06-16** — the Apertus-70B single-node
real-scenario capacity run, executed on **both SLURM (clariden) and K8s (breithorn)** as the
baseline cell of the KV-cache grid (report `reports/2026-06-16_apertus70b-baseline-single-node/`:
λ\*=0.5, gsm8k 0.72, platform parity). Next on the critical path: **E4** (the KV-offload/dtype
feature-effect sweeps — configs + early-stop infra ready, baseline done) and the **Kimi-K2.6
flagship (E3)**, whose Ray-in-image blocker is resolved in-repo
(`nvidia-gh200-vllm-0.23.0-net.v1`, built + pushed, now ships Ray+NIXL+LMCache).
...
### M7 — Benchmarker orchestrator (M–L) *(added by review H1)* — ✅ laptop half done 2026-06-13 (run_experiment phase driver + EngineLauncher/QualityEvaluator seams + Slurm/K8s launchers + CLI + 7 mock-integration tests; planner gained `--deployment-index`; nested-sbatch / squeue / kubectl paths validated on-cluster at E1/E5; the M11 quality evaluator slots into the seam via `main.py`. **Extended 2026-06-16**: sustained-queue **adaptive λ-sweep early-stop** (`rate_sweep`, §12.2), **`BackendConfig.env`** engine-env passthrough (EDF + K8s pod env), **pre-staged dataset-pool reuse** (generate once, reuse across a sweep's cells), primer clamp to `max_model_len`; ExternalEndpointLauncher (K8s) cluster-validated at E3a)
...
| **E4** | **Feature-effect sweeps** on the E3 workload: `enable_prefix_caching` on/off, `kv_offloading_size`, `kv_cache_dtype`, `session_affinity` vs `random` (multi-instance); speculative decoding on Apertus-70B chat/long-context (§9.2 pairing, §17.2 placements; `spec_accept_rate` captured via the M2 scraper) | E3 baseline | Marginal effect of each feature on λ\*/users quantified per §16.1, **with the §13.5 capacity-vs-quality pairing for quality-impacting knobs** (quantization / KV dtype) — the flagship *"N× more users at −M pts"* report; findings recorded in §17. **Infra ready 2026-06-16**: KV-grid configs (`apertus70b-kvgrid-single-node-{slurm,k8s}.yaml` — offload {off, native} × dtype {default, fp8}) + `BackendConfig.env` passthrough + sustained-queue adaptive early-stop (§12.2) + shared-pool reuse; baseline cell done (E3a). **Before the offload cells**: right-size `kv_offloading_size` (400 GiB OOMs the GH200 host at 256K) + build the parallel campaign driver; LMCache is a deferred 3rd offload axis (needs the KV-connector plumbing). |
| **E5** | **Platform comparison**: E3 workload, SLURM (`clariden`) vs K8s (`breithorn`), same GH200 hardware, engine, config; the SLURM Benchmarker drives the K8s engine (M6/M7 K8s engine path) | E3; E2c | Per-platform overlay report isolating the platform contribution (§16.1); "is K8s slower than it could be?" answered with telemetry-backed evidence. **Single-node baseline slice DONE 2026-06-16** (Apertus-70B, SLURM vs K8s, identical image/config/offered-load): **platform parity** — same λ\*=0.5, identical gsm8k Stage-B 0.7187; K8s adds only ~14 ms p50 / ~108 ms chat-p95 ingress overhead at λ\*, no capacity/quality penalty; report `reports/2026-06-16_apertus70b-baseline-single-node/`. K8s cross-cluster instrumentation gaps surfaced (prechecks values / loading breakdown / hardware_stats not persisted — TODOs). Full E5 (E3/Kimi workload, multi-node) remains. |
