...
# This script is executed by every rank under `srun --mpi=pmix`; nccl-tests'
# binaries use PMIx to discover rank/size and only rank 0 prints results.
#
...
# scatter_perf, gather_perf, reduce_perf, hypercube_perf — add them to the
# COLLECTIVES list if you need them (e.g. sendrecv_perf for pipeline-parallel
# serving).
#
# Required env:
...
#                        "8G"). Lower bound is fixed at 8 B; factor is x2.
# Optional:
...
BUILD_DIR="${NCCL_TESTS_CACHE}/v${NCCL_TESTS_VERSION}/build"
...
for c in ${COLLECTIVES}; do
    ...
    if [ ! -x "${bin}" ]; then
        rank0 && echo "[run] missing ${bin} — rebuild the cache" >&2
        fail=1
        continue
    fi
    rank0 && echo "===== ${c} ====="
    if ! "${bin}" -b 8 -e "${MSG_END}" -f 2 -g 1; then
        rank0 && echo "[run] ${c} failed (non-zero exit)" >&2
        fail=1
    fi
done
